...
<section id="programs" className="bg-black py-20 sm:py-24">
            <div className="container mx-auto px-6 lg:px-8">
                <div className="text-center mb-12">
                    ...
                    <p className="text-gray-400 mt-3 max-w-2xl mx-auto">Specialized programs to fuel every stage of your business journey.</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {programs.map((program) => (
                        <div key={program.id} className="bg-[#1a1a1a] rounded-lg flex flex-col p-6 text-left">
                            <div className="overflow-hidden rounded-md mb-6">
                                <Link href={`/learn-more/${program.id}`}>
                                    <Image src={program.image} alt={program.title} width={600} height={600} className="w-full h-auto object-cover transform hover:scale-105 transition-transform duration-300"/>
                                </Link>
                            </div>
                            <div className="flex-grow flex flex-col">
                                <h3 className="text-xl font-bold text-white mb-2"><Link href={`/learn-more/${program.id}`} className="hover:text-red-500 transition-colors">{program.title}</Link></h3>
                                <p className="text-gray-400 text-sm leading-relaxed mb-6 flex-grow">{program.description}</p>
                                <div className="mt-auto">
                                    <Link href={`/learn-more/${program.id}`} className="inline-block w-full text-center px-5 py-2 text-sm font-medium border border-gray-700 rounded-full hover:bg-red-600 hover:border-red-600 transition-colors">{program.ctaText}</Link>
                                </div>
                            </div>
                        </div>
                    ))}
